...
public class BankingService {
    ...
    public Optional<String> checkExistingCompleteAccountAndActivate(CustomerRequest request) {
        ...
        if (account.getMbActivationCode() == null || account.getMbActivationCode().isEmpty()) {
            ...
            try {
                String activationCode = mobileBankingPort.activate(request, account.getCif(),
     ...
     * Extracts both CIF and MNEMONIC from a SINGLE coreBankingPort.createCustomer() call.
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    try {
                        ...
                        log.info("✗ Customer NOT found in system. Retrying coreBankingPort.createCustomer() - Attempt {} of {}", attempt + 1, maxRetries);
    ...
    private String createAccountWithRetry(CustomerRequest request, Map<String, String> customerInfo, String cif, String currency) {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                log.info("Calling coreBankingPort.createAccount({}) - Attempt {} of {}", currency, attempt, maxRetries);
                return createAccount(request, cif, currency);
            } catch (Exception e) {
                ...
                if (attempt < maxRetries) {
                    ...
                    try {
                        ...
                        log.info("✗ {} account NOT found in system. Retrying coreBankingPort.createAccount({}) - Attempt {} of {}", currency, currency, attempt + 1, maxRetries);
                    } catch (Exception verifyError) {
                        log.error("Failed to verify customer info before retry: {}", verifyError.getMessage());
                        throw new AccountCreationException("Customer verification failed before retry");
                    }
    ...
    private String createAccount(CustomerRequest request, String cif, String currency) {
        try {
            ...
            Document response = coreBankingPort.createAccount(request, cif, currency);
            if (response == null) {
