$(document).ready(function () {
    ...
    $('#dateOfBirth').datetimepicker({
        lang: 'en', timepicker: false, format: 'd/m/Y', formatDate: 'Y/m/d'
    });
...
const messages = {
    ENG: {
        success: "Successfully..!", fail: "Failed..!", alreadyExists: "Already Existed..!"
    }, KH: {
        success: "ជោគជ័យ..!", fail: "បរាជ័យ..!", alreadyExists: "មានគណនីរួចរាល់..!"
    }
};
                ...
                success: function (response) {
                    ...
                    if (response.error == 0) {
                        if (response.data !== null) {
                            ...
                            Swal.fire({
                                title: "Failed...!", text: response.message, icon: "error"
                            });
                        }
                    } else {
                        hideLoading();
...
function ValidateNidFace() {
    ...
    function makeAjaxRequest() {
        const json = {
            ...
            }, faceImg: selfieImageValue, idImage: legalImageValue
        };

        $.ajax({
            type: "POST",
            url: "api/v1/eKYC/validateNidFace",
            contentType: 'application/json',
            dataType: 'json',
            data: JSON.stringify(json),
            success: function (response) {
        ...
        if (faceDocumentScore >= 0.50 && faceMoiScore >= 0.50 && NidScore >= 0) {
            ...
            if (incorrectFields.includes("lastNameKh") || incorrectFields.includes("firstNameKh") || incorrectFields.includes("dob") || incorrectFields.includes("gender") || incorrectFields.includes("lastNameEn") || incorrectFields.includes("firstNameEn")) {
                ...
                if (language === 'KH') {
                    ...
                    Swal.fire({
                        icon: "warning", title: "បរាជ័យ", html: html
                    });
                } else {
                    ...
                    Swal.fire({
                        icon: "warning", title: "Failed..!", html: html
                    });
                }
            } else {
                hideLoading();
...
function handleUploadFailure(title, message) {
    hideLoading();
    Swal.fire({
        title: title, text: message, icon: "error", timer: 6000,
    });
}
...
function determineModalToShow() {
    ...
    if (isCheckAddressCustomerFound === 0 && isCheckPOBAddressCustomerFound === 0) {
        ...
        if (language === 'KH') {
            Toast.fire({
                icon: 'success', title: "ព័ត៌មានរបស់លោកអ្នកត្រឹមត្រូវ"
            });
        } else {
            Toast.fire({
                icon: 'success', title: "Your information is correct"
            });
        }
