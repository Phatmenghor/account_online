...
function filterMenuByRole(items: StaticMenuItem[], role: RoleEnum): StaticMenuItem[] {
  return items
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b px-3 relative">
          {isOpen ? (
            ...
            <Link href={ROUTES.DASHBOARD.INDEX} className="flex items-center justify-center w-full">
              <img src={AppIcons.APP.APP_LOGO} alt="Logo" className="w-10 h-10" />
            </Link>
          )}
        ...
        <div className="border-t p-3">
          {isLoadingUser ? (
            <div className="animate-pulse flex flex-col gap-2">
              <div className={cn("h-10 bg-slate-200 rounded-md dark:bg-slate-700", isOpen ? "w-full" : "w-10 mx-auto")} />
              {isOpen && <div className="h-4 w-3/4 bg-slate-200 rounded-md dark:bg-slate-700" />}
            </div>
          ) : (
